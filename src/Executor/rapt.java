...
public class rapt implements test {
    ...
    public void executeTestCase(String program_name, String version, int repeatTimes) {
        ...
        for (int i = 0; i < constant.repeatnum; i++) {
            System.out.println(program_name + " " + version + " use RAPT ; now testing " + String.valueOf(i + 1));

            // initialize RAPT
            RAPT rapt = new RAPT();
            rapt.setParameters4RAPT(program_name, version);
            rapt.initializeRAPT(constant.get_num_of_partition(program_name));
